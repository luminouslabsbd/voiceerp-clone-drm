...
router.post('/',
  ...
  async(req, res) => {
    ...
    try {
      ...
      const {lookupAccountDetails, lookupCarrierByPhoneNumber, lookupCarrier, lookupVoipCarrierBySid} = dbUtils(logger, srf);
      const {
        lookupAppBySid
      ...
      const opts = {
        // Don't set callingNumber here, we'll set From header directly
      ...
      // Set From header for all calls (with localhost so SBC can replace it)
      if (!opts.headers['From'] && !opts.headers['from']) {
        opts.headers['From'] = `<sip:${restDial.from}@localhost>`;
        opts.headers['from'] = `<sip:${restDial.from}@localhost>`;
      }

      // Also set callingNumber so Drachtio creates the From header with localhost
      opts.callingNumber = restDial.from;
      ...
      if (target.type === 'phone' && target.trunk) {
        ...
        if (voip_carrier_sid) {
          ...
          // Fetch carrier details to get register_from_user and register_from_domain
          try {
            const voipCarrier = await lookupVoipCarrierBySid(voip_carrier_sid);
            logger.info(`createCall: fetched voipCarrier: ${JSON.stringify(voipCarrier)}`);
            // Handle array response from lookupVoipCarrierBySid
            const carrier = Array.isArray(voipCarrier) ? voipCarrier[0] : voipCarrier;
            if (carrier && carrier.register_from_user && carrier.register_from_domain) {
              target.from = {
                user: carrier.register_from_user,
                host: carrier.register_from_domain
              };
              logger.info(`createCall: set from header to ${target.from.user}@${target.from.host} for carrier ${target.trunk}`);
              opts.headers['X-Preferred-From-User'] = target.from.user;
              opts.headers['X-Preferred-From-Host'] = target.from.host;
              logger.info(`createCall: set X-Preferred-From-User to ${target.from.user}`);
              logger.info(`createCall: set X-Preferred-From-Host to ${target.from.host}`);
            }
          } catch (err) {
            logger.error({err}, 'createCall: error fetching carrier details for from header');
          }
        }
      }
     ...
     */
      ...
      try {
        logger.info(`createCall: about to call createUAC with opts: ${JSON.stringify({...opts, headers: opts.headers})}`);
