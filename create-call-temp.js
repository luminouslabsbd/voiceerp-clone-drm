    ...
    try {
      ...
      if (target.type === 'phone' && target.trunk) {
        ...
        if (voip_carrier_sid) {
          ...
          // Add from object with register_from_user and register_from_domain from carrier config
          try {
            ...
            if (carrier && carrier.register_from_user && carrier.register_from_domain) {
              ...
              // Set From header directly for Drachtio
              opts.headers['From'] = `<sip:${target.from.user}@${target.from.host}>`;
              opts.headers['Contact'] = `<sip:${target.from.user}@${target.from.host}>`;
              logger.info(`createCall: set From header to ${opts.headers['From']}`);
              logger.info(`createCall: set Contact header to ${opts.headers['Contact']}`);
              logger.info(`createCall: full opts.headers: ${JSON.stringify(opts.headers)}`);
              // Set From header directly for Drachtio
              opts.headers['From'] = `<sip:${target.from.user}@${target.from.host}>`;
              opts.headers['Contact'] = `<sip:${target.from.user}@${target.from.host}>`;
              logger.info(`createCall: set From header to ${opts.headers['From']}`);
